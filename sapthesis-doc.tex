...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{quick_glance.png}
...
manages the catalog of available tools.  At startup, it discovers available tools using a
convention-over-configuration approach and registers each one
automatically.  For each tool, the registry stores the tool's name,
...
The \emph{runtime wrapper} is a decorator applied to each tool function.
...
The test runner uses a marker-based classification scheme.  Five primary
markers govern execution scope: \emph{unit} tests execute in isolation
with mocked dependencies and complete in milliseconds; \emph{integration}
tests exercise component interactions with real or simulated
infrastructure; \emph{end-to-end} tests send HTTP requests through the
full API stack and validate complete workflows;
\emph{security} tests target authentication, authorisation, tenant
isolation, injection prevention, and PII scrubbing; and
\emph{performance} tests measure latency and throughput under load and
are excluded from the default test run to avoid inflating execution time.
Two additional markers isolate long-running NL-to-Cypher tests that depend
on LLM availability: one for a representative subset (approximately twenty
minutes) and one for the full catalog (approximately ninety minutes).

The test infrastructure provides reusable fixtures at the session and
function scope.  Session-scoped fixtures initialise shared resources
(database connections, identity-provider key pairs for token generation,
application instances) once per test session.  Function-scoped fixtures
provide isolated state (fresh database transactions that are rolled back
after each test, namespaced in-memory store keys, deterministic token
...
\texttt{security.describe\_principal}) are registered via
\texttt{@mcp\_tool} decorators but are not yet declared in the JSON
manifest file.  Table~\ref{tab:app-tool-inventory} lists every tool
...
graph.secure\_query  & Graph & tools:basic & Natural-language to Cypher with multi-layer safety validation \\
...
graph.generate\_cypher & Graph & tools:basic & Generate Cypher from natural-language descriptions \\
\hline
...
\begin{tabularx}{\textwidth}{@{}l l X@{}}
    ...
    POST   & \texttt{/v1/jobs/\{id\}/retry}  & Retry a failed job \\
    GET    & \texttt{/v1/jobs/stats}          & Queue-depth and backlog statistics \\
    \hline
\end{tabularx}
\end{table}
...
marker-based filters (e.g., selecting only performance or security
markers).
