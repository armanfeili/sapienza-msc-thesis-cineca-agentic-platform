...
% [x] Verify no contradictory statements across chapters (tool counts, endpoint counts, test counts) - VERIFIED: All counts consistent (34 tools, 76 endpoints, 3,000+ tests, ~411,700 LOC, 8,263 orchestrator lines)
% [x] Reconcile quantitative claims to single source-of-truth - DONE: Standardized to README.md/slides.md authoritative values
% [x] Add TODO: verify latest counts if uncertain - DONE: Quantitative claims standardized and verified against README.md

% ============================================================================
% 6. METRICS, EVALUATION, AND CLAIMS HYGIENE
% ============================================================================
% [x] Verify quantitative claims are consistent: "34 tools", "76 endpoints", "3,000+ tests", "~411,700 LOC", "8,263 orchestrator lines" - DONE (synchronized with README.md)
...
% Values synchronized with README.md and slides.md (authoritative sources)
...
\begin{description}
    \item[Functional Testing:] Automated tests verifying that implemented features meet specifications. The test suite includes over 3,000 test functions covering all major subsystems.
...
\begin{itemize}
    \item Approximately 411,700 total lines of code across all formats
    \item Over 3,000 test functions across 236+ test files
...
Describes the technical implementation. Covers the technology stack (Python, FastAPI, PostgreSQL, Redis, Memgraph, Next.js, Streamlit), codebase organization with approximately 411,700 total lines of code, coding standards, testing strategy with over 3,000 test functions, and platform engineering utilities (pagination, idempotency, JSON handling, deprecation framework).
...
This chapter presents the comprehensive architectural design of the CINECA Agentic Platform, an enterprise-grade AI agent orchestration system designed to bridge the gap between natural language queries and secure, auditable database operations. The platform enables researchers and operators to interact with complex graph databases using conversational language while maintaining enterprise-grade security, multi-tenancy, and full compliance auditability.

%------------------------------------------------------------------------------
\section{Executive Summary}
\label{sec:arch-executive-summary}

\subsection{Platform Overview}
\label{subsec:platform-overview}

The CINECA Agentic Platform is a production-grade AI agent orchestration system that provides the following key capabilities:

\begin{itemize}
    \item \textbf{NL-to-Cypher}: Translate natural language to validated Cypher queries for graph database interactions
    \item \textbf{Multi-Tenant}: Complete data isolation between organizations with tenant-specific configurations
    \item \textbf{Model-Agnostic}: Switch between OpenAI, Azure OpenAI, and Ollama without code changes
    \item \textbf{34 MCP Tools}: Extensible tool ecosystem with RBAC enforcement across 17 categories
    \item \textbf{Full Observability}: OpenTelemetry tracing, Prometheus metrics, and Grafana dashboards
    \item \textbf{Resilience}: Circuit breakers, exponential backoff retries, and provider fallback chains
\end{itemize}

\subsection{Platform Statistics}
\label{subsec:platform-stats}

Table~\ref{tab:platform-scale} summarizes the scale and scope of the platform implementation.

\begin{table}[htbp]
\centering
\caption{Platform scale metrics}
\label{tab:platform-scale}
\begin{tabular}{@{}ll@{}}
\toprule
...
Lines of Code & $\sim$411,700 \\
MCP Tools & 34 \\
Tool Categories & 17 \\
Infrastructure Components & 16+ \\
Database Architecture & 3-Database \\
\bottomrule
\end{tabular}
\end{table}
...
The platform is organized into 13 distinct architectural layers, each responsible for specific functionality. Figure~\ref{fig:simplified-architecture} shows the simplified architecture diagram, while Figure~\ref{fig:full-architecture} presents the complete system with all components.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/DIAGRAM_SIMPLE.png}
    \caption{Simplified architecture diagram of the Cineca Agentic Platform showing the main components and data flow.}
    \label{fig:simplified-architecture}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/DIAGRAM_FULL_WORKFLOWS.png}
    \caption{Full architecture diagram of the Cineca Agentic Platform showing both execution workflows, security middleware, service layer, data infrastructure, and observability components.}
...
\begin{enumerate}
    \item \textbf{Layer 1: Gateway \& Authentication} --- NGINX reverse proxy and OIDC/Auth0 identity provider
    \item \textbf{Layer 2: Security Middleware} --- 8-layer security stack including CORS, JWT auth, rate limiting, and guards
    \item \textbf{Layer 3: API Layer} --- FastAPI routers exposing 76 versioned endpoints
    \item \textbf{Layer 4: Execution Workflows} --- Dual workflow system for agent runs and long-running jobs
    \item \textbf{Layer 5: Service Layer} --- Orchestrator service with 4-phase execution pipeline
    \item \textbf{Layer 6: LLM Providers} --- Model-agnostic adapter layer for multiple AI providers
    \item \textbf{Layer 7: MCP Runtime \& Tools} --- 34 tools across 17 categories with RBAC enforcement
    \item \textbf{Layer 8: NL-to-Cypher Pipeline} --- 6-stage pipeline for natural language to graph queries
    \item \textbf{Layer 9: Data Layer} --- Three-database architecture (Redis, PostgreSQL, Memgraph)
    \item \textbf{Layer 10: Adapters} --- Swappable interfaces for external services
    \item \textbf{Layer 11: Resilience Framework} --- Circuit breakers, retries, and fallback chains
    \item \textbf{Layer 12: Background Framework} --- APScheduler tasks for maintenance operations
    \item \textbf{Layer 13: Observability} --- OpenTelemetry, Prometheus, and Grafana integration
\end{enumerate}

%------------------------------------------------------------------------------
\section{Layer 1: Identity and Authentication}
\label{sec:layer1-auth}

\subsection{Identity Provider (OIDC / Auth0)}
\label{subsec:identity-provider}

The platform uses industry-standard OAuth 2.0 / OpenID Connect for authentication. The identity provider handles user authentication and issues JWT tokens containing essential claims for authorization decisions.

\subsubsection{JWT Token Claims}

Each authenticated request carries a JWT token with the following claims:

\begin{itemize}
    \item \texttt{sub}: Unique user identifier
    \item \texttt{tenant\_id}: Organization the user belongs to
    \item \texttt{roles[]}: Role assignments (admin, operator, user, viewer)
    \item \texttt{scopes[]}: Permission scopes (read, write, graph:read, graph:write, tools:invoke)
    \item \texttt{exp}: Token expiration timestamp
    \item \texttt{iat}: Token issued-at timestamp
\end{itemize}

\subsubsection{Principal Object}

After JWT validation, the system constructs a Principal object that encapsulates the authenticated user's identity and permissions:

\begin{lstlisting}[language=Python, caption={Principal object structure}]
@dataclass
class Principal:
    sub: str
    tenant_id: str
    roles: List[str]  # [admin|operator|user|viewer]
    scopes: List[str]  # [user:me, admin:all, tools:invoke:*, graph:*]
\end{lstlisting}
...
\subsubsection{Agent Chat UI (Next.js/React)}

The Agent Chat UI serves as the end-user interface for AI agent interactions, implementing Workflow A (synchronous agent runs):

\begin{itemize}
    \item JWT-based authentication with automatic token refresh
    \item Server-Sent Events (SSE) streaming for real-time updates
    \item Agent runs and steps visualization
    \item Real-time polling for status updates
\end{itemize}

\subsubsection{Control Panel UI (Streamlit)}

The Control Panel provides administrative capabilities for platform management, implementing Workflow B (long-running jobs):

\begin{itemize}
    \item Jobs, models, and tools management
    \item Graph and NL-to-Cypher experiments
    \item SSE streaming for job events
    \item ETL, backup, and maintenance triggers
\end{itemize}

\subsection{Reverse Proxy / API Gateway (NGINX)}
\label{subsec:nginx-gateway}

NGINX serves as the API gateway providing:

\begin{itemize}
    \item TLS termination for secure communications
    \item Request routing to backend services
    \item CORS handling for cross-origin requests
    \item Load balancing across application instances
\end{itemize}

%------------------------------------------------------------------------------
\section{Layer 2: Security Middleware Stack}
\label{sec:layer2-security}

Every request passes through eight security middleware layers before reaching business logic. This defense-in-depth approach ensures comprehensive protection at multiple levels.

\subsection{Middleware Pipeline}
\label{subsec:middleware-pipeline}

The security middleware executes in the following order:

\begin{enumerate}
    \item \textbf{CORS Handler}: Validates allowed origins and handles preflight requests
    \item \textbf{Trace Context}: Injects/extracts W3C trace-context headers for distributed tracing
    \item \textbf{Auth JWT}: Validates Bearer tokens via JWKS endpoint and builds Principal objects
    \item \textbf{Rate Limiter}: Enforces per-user, per-tenant, and per-endpoint rate limits
    \item \textbf{Tenant Resolver}: Resolves tenant configuration and injects tenant filters
    \item \textbf{Input Guard}: Validates and sanitizes all request inputs
    \item \textbf{Output Guard}: Scrubs PII and applies compliance filtering
    \item \textbf{Error Handler}: Normalizes exceptions and attaches correlation IDs
\end{enumerate}

\subsection{RBAC Permission Matrix}
\label{subsec:rbac-matrix}

Table~\ref{tab:rbac-matrix} shows the role-based access control permissions for different resource types.

\begin{table}[htbp]
\centering
\caption{RBAC permission matrix by role}
\label{tab:rbac-matrix}
\begin{tabular}{@{}lccccc@{}}
\toprule
\textbf{Role} & \textbf{agent-runs} & \textbf{jobs} & \textbf{tools} & \textbf{tenants} & \textbf{graph:write} \\
\midrule
admin & CRUD & CRUD & CRUD & CRUD & \checkmark \\
...
\label{subsec:rate-limiter}

The rate limiter uses Redis-backed storage with multi-dimensional limiting:

\begin{itemize}
    \item \texttt{ratelimit:user:\{sub\}}: Per-user limits
    \item \texttt{ratelimit:tenant:\{tenant\_id\}}: Per-tenant limits
    \item \texttt{ratelimit:endpoint:\{method\}:\{path\}}: Per-endpoint limits
\end{itemize}

Response headers communicate limit status:
\begin{itemize}
...
\label{subsec:input-guard}

The Input Guard provides multiple protection mechanisms:

\begin{itemize}
    \item Pydantic schema validation for all request bodies
    \item SQL injection detection (DROP TABLE, UNION SELECT patterns)
    \item Cypher injection detection for graph database attacks
    \item Command injection detection (shell metacharacters)
...
The Output Guard ensures compliance and privacy:

\begin{itemize}
    \item PII scrubbing (email addresses, phone numbers, SSNs)
    \item Content filtering for sensitive patterns
    \item Response size validation and truncation
    \item Audit logging with correlation IDs
\end{itemize}

%------------------------------------------------------------------------------
\section{Layer 3: API Layer}
\label{sec:layer3-api}

The platform exposes 76 endpoints organized into 16 versioned API groups. Table~\ref{tab:api-endpoints} summarizes the major endpoint categories.
...
%------------------------------------------------------------------------------
\section{Layer 4: Execution Workflows}
\label{sec:layer4-workflows}

The platform supports two distinct execution paths optimized for different use cases.

\subsection{Workflow Comparison}
\label{subsec:workflow-comparison}

Table~\ref{tab:workflow-comparison} compares the two workflow types.

\begin{table}[htbp]
\centering
\caption{Comparison of execution workflows}
\label{tab:workflow-comparison}
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Aspect} & \textbf{Workflow A: Agent Runs} & \textbf{Workflow B: Long Jobs} \\
\midrule
Execution & In-process & Worker pool \\
Via & BackgroundTasks & Redis queue \\
Duration & Short/Medium & Long \\
Fault-tolerance & Basic & High \\
Scaling & Vertical & Horizontal \\
Use Case & Chat, quick Q\&A & ETL, batch, heavy \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Workflow A: Agent Runs}
\label{subsec:workflow-a}

Agent runs follow a synchronous execution model with the following state machine:

\begin{center}
\texttt{queued} $\rightarrow$ \texttt{running} $\rightarrow$ \texttt{succeeded} $|$ \texttt{failed}
\end{center}

Key endpoints:
\begin{itemize}
    \item \texttt{POST /v1/agent-runs}: Start synchronous execution
    \item \texttt{POST /v1/agent-runs?use\_jobs=true}: Route to async worker
    \item \texttt{GET /v1/agent-runs/\{id\}}: Retrieve run with ETag caching
    \item \texttt{GET /v1/agent-runs/\{id\}/steps}: Get execution trace
\end{itemize}

\subsection{Workflow B: Long-Running Jobs}
\label{subsec:workflow-b}

Long-running jobs use Redis queues and dedicated workers:

\begin{center}
\texttt{queued} $\rightarrow$ \texttt{running} $\rightarrow$ \texttt{finished} $|$ \texttt{failed} $|$ \texttt{cancelled}
\end{center}

Key endpoints:
\begin{itemize}
    \item \texttt{POST /v1/jobs}: Create job (returns 202 + Location header)
    \item \texttt{GET /v1/jobs/\{id\}}: Get status and results
    \item \texttt{GET /v1/jobs/\{id\}/events}: SSE streaming
    \item \texttt{DELETE /v1/jobs/\{id\}}: Cancel via Redis Lua script
\end{itemize}

Job types include: demo, test, long-running, and agent.run.

\subsection{Job Creation Flow}
\label{subsec:job-creation-flow}

The job creation process follows these steps:

\begin{enumerate}
    \item \textbf{Validate}: Pydantic schema + Job-type JSON Schema validation
    \item \textbf{Idempotency Check}: Check Redis + PostgreSQL for existing job
    \item \textbf{Create Record}: INSERT INTO jobs with status='queued'
    \item \textbf{Enqueue}: LPUSH job\_id to Redis jobs:queue:\{type\}
    \item \textbf{Cache Key}: Store idempotency:\{key\} $\rightarrow$ job\_id
    \item \textbf{Respond}: HTTP 202 Accepted with Location header
\end{enumerate}

\subsection{Worker Processing Loop}
\label{subsec:worker-processing}

Workers process jobs through the following loop:

\begin{enumerate}
    \item \textbf{Pop Job}: BRPOP from jobs:queue:\{type\} (blocking wait)
    \item \textbf{Load Metadata}: SELECT from jobs table
    \item \textbf{Check Cancel}: EXISTS jobs:cancel:\{id\}
    \item \textbf{Transition Status}: queued $\rightarrow$ running, emit SSE event
    \item \textbf{Heartbeat}: SETEX jobs:heartbeat:\{id\} every 30 seconds
    \item \textbf{Execute Handler}: Route to handler by job type
    \item \textbf{Emit Events}: RPUSH to jobs:events:\{id\} for SSE stream
    \item \textbf{Finalize}: Update status, persist result to PostgreSQL
\end{enumerate}

%------------------------------------------------------------------------------
\section{Layer 5: Service Layer (Orchestrator)}
\label{sec:layer5-orchestrator}

The Orchestrator Service is the core execution engine, processing prompts through four phases.

\subsection{Phase 1: Intent Classification}
\label{subsec:phase1-intent}

The orchestrator first classifies the user's intent:

\begin{itemize}
    \item \textbf{CHAT}: Conversational responses (``What is ML?'')
    \item \textbf{GRAPH}: NL-to-Cypher queries (``Which institutions?'')
    \item \textbf{SECURITY}: Permission/access questions
    \item \textbf{ADMIN}: Administrative write operations
    \item \textbf{DANGEROUS}: Destructive operations (refused, offer EXPLAIN)
    \item \textbf{EXPLAIN}: Query analysis without execution
\end{itemize}

\subsection{Phase 2: TODO Planning}
\label{subsec:phase2-planning}

Based on intent, the orchestrator creates a plan:

\begin{itemize}
    \item \textbf{full}: LLM generates detailed plan for each TODO
    \item \textbf{optional}: Try direct execution; fall back to planning
    \item \textbf{none}: Deterministic execution without LLM planning
\end{itemize}

Output: TODO Array with actions and parameters.

\subsection{Phase 3: Step Execution}
\label{subsec:phase3-execution}

For each TODO item:

\begin{enumerate}
    \item Call LLM provider via resilient adapter layer
    \item Invoke MCP tools with RBAC check
    \item Execute NL-to-Cypher pipeline if GRAPH intent
    \item Persist step to PostgreSQL immediately
    \item Use Redis for session state and cancellation checks
\end{enumerate}

\subsection{Phase 4: Finalization}
\label{subsec:phase4-finalization}

The final phase produces the response:

\begin{enumerate}
    \item Build response aggregating results from all steps
    \item Create canonical output with goal, steps, outputs, todos, metrics
    \item Normalize to text + optional JSON payload
    \item Apply PII scrubbing and output guard
    \item Persist final state to PostgreSQL
    \item Emit Prometheus metrics and OpenTelemetry traces
\end{enumerate}

%------------------------------------------------------------------------------
\section{Layer 6: LLM Providers}
\label{sec:layer6-llm}

The platform implements a model-agnostic architecture supporting multiple LLM providers.

\subsection{Supported Providers}
\label{subsec:llm-providers}

\subsubsection{Ollama (Local)}

Benefits:
\begin{itemize}
    \item Data sovereignty (prompts never leave infrastructure)
    \item No API costs (compute-only)
    \item Offline capable
    \item Customization through fine-tuning
\end{itemize}

Supported models: Phi-3 Mini, Mistral/Mixtral, LLaMA 2/3, Qwen, Gemma.

\subsubsection{OpenAI (Cloud)}
...
Benefits: Compliance certifications, regional data residency, SLA guarantees.

\subsection{Provider Selection Hierarchy}
\label{subsec:provider-selection}

Provider selection follows a hierarchical resolution:

\begin{enumerate}
    \item Request-level override (user specifies in API call)
    \item Session-level default (team/user preference)
    \item Tenant-level default (organization policy)
    \item Global platform default (system-wide fallback)
\end{enumerate}

%------------------------------------------------------------------------------
\section{Layer 7: MCP Runtime and Tools}
\label{sec:layer7-mcp}

The Model Context Protocol (MCP) runtime manages 34 tools across 17 categories.

\subsection{Tool Categories}
\label{subsec:tool-categories}

Table~\ref{tab:tool-categories} lists the available tool categories.

\begin{table}[htbp]
\centering
\caption{MCP tool categories}
\label{tab:tool-categories}
\begin{tabular}{@{}ll@{}}
\toprule
\textbf{Category} & \textbf{Tools} \\
\midrule
GRAPH & query, secure\_query, generate\_cypher, schema, explain \\
SECURITY & describe\_principal, allowed\_operations, validate \\
...
ADMIN & create\_index, drop\_index, constraint \\
ETL & import, export, transform \\
CRUD & create\_node, update\_node, delete\_node \\
EXPORT & csv, json, cypher \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Tool Invocation Flow}
\label{subsec:tool-invocation}

Each tool invocation follows this flow:

\begin{enumerate}
    \item \textbf{Input Validation}: JSON Schema validation (invalid $\rightarrow$ 400)
    \item \textbf{RBAC Check}: Verify caller has required scopes (missing $\rightarrow$ 403)
    \item \textbf{Execute Tool}: Perform actual operation (error $\rightarrow$ 500)
    \item \textbf{Audit Log}: Log all attempts including failures
    \item \textbf{Return Result}: Success response with data
\end{enumerate}

\subsection{Tool Manifest Schema}
\label{subsec:tool-manifest}

Each tool is defined by a manifest:

\begin{lstlisting}[language=Python, caption={Tool manifest example}]
{
...
%------------------------------------------------------------------------------
\section{Layer 8: NL-to-Cypher Pipeline}
\label{sec:layer8-nl2cypher}

The NL-to-Cypher pipeline translates natural language queries into validated Cypher queries through six stages.

\subsection{Pipeline Stages}
\label{subsec:pipeline-stages}

\begin{enumerate}
    \item \textbf{Normalize}: Lowercase, remove fillers, sanitize input, extract entities
    
    Example: ``Hey, how many Blast nodes??'' $\rightarrow$ ``count blast nodes''
    
    \item \textbf{Catalog Lookup}: Check if query matches known patterns for cached Cypher
    
    \item \textbf{Generate Cypher}: LLM generates Cypher with schema context
    
    \item \textbf{Validate (6-Layer)}: 
    \begin{itemize}
        \item Syntax check
        \item Tenant boundary enforcement
        \item Query depth limits
        \item Timeout guards
        \item Result size caps
        \item Read-only enforcement
    \end{itemize}
    
    \item \textbf{Execute}: Run validated Cypher on Memgraph via graph.secure\_query
    
    \item \textbf{Summarize}: LLM converts results to natural language
\end{enumerate}

\subsection{End-to-End Example}
\label{subsec:nl2cypher-example}

\begin{lstlisting}[caption={NL-to-Cypher pipeline example}]
...
%------------------------------------------------------------------------------
\section{Layer 9: Data Layer}
\label{sec:layer9-data}

The platform employs a three-database architecture optimized for different data access patterns.

\subsection{Database Comparison}
\label{subsec:db-comparison}

Table~\ref{tab:database-comparison} compares the three databases.

\begin{table}[htbp]
\centering
\caption{Three-database architecture comparison}
\label{tab:database-comparison}
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{Aspect} & \textbf{Redis} & \textbf{PostgreSQL} & \textbf{Memgraph} \\
\midrule
Role & Speed layer & Durability layer & Relationship layer \\
Use Case & Cache, queues & State, audit & Graph queries \\
Data Model & Key-value & Relational & Property graph \\
Query & Commands & SQL & Cypher \\
Persistence & Optional & Always (WAL) & Always (WAL) \\
...
\subsection{Redis (Speed Layer)}
\label{subsec:redis-layer}

Redis provides high-speed data access for:

\begin{itemize}
    \item \textbf{Cache}: Session data, tenant configs, model defaults
    \item \textbf{Queues}: Job queues (LPUSH/BRPOP), SSE event buffers, job state
    \item \textbf{Rate Limiting}: Per-user, per-tenant, per-endpoint limits
    \item \textbf{Control}: Idempotency mapping, circuit breaker state, cancellation flags
\end{itemize}

\subsection{PostgreSQL (Durability Layer)}
\label{subsec:postgresql-layer}

PostgreSQL stores persistent state:

\begin{itemize}
    \item \texttt{tenants}: Multi-tenant configurations
    \item \texttt{agent\_runs}: Agent run records with status
    \item \texttt{agent\_sessions}: Conversation state
    \item \texttt{agent\_steps}: Per-step execution records
    \item \texttt{jobs}: Async job records
    \item \texttt{job\_events}: Job progress events (append-only)
    \item \texttt{tools}: MCP tool definitions
    \item \texttt{tool\_invocations}: Tool execution audit trail
    \item \texttt{audit\_logs}: Compliance trail (all actions)
\end{itemize}

\subsection{Memgraph (Graph Layer)}
\label{subsec:memgraph-layer}

Memgraph provides graph database capabilities for the bioinformatics domain with 14 node types and 4 relationship types:

\textbf{Node Types}: User, Institution, Task, File, Dataset, Sample, Experiment, Publication, Gene, Protein, Pathway, Tool, Workflow, Result.

\textbf{Relationship Types}: WORKS\_AT, RUNS, INPUT, OUTPUT.

%------------------------------------------------------------------------------
\section{Layer 10: Adapters}
\label{sec:layer10-adapters}

Adapters provide swappable interfaces for external services, enabling easy replacement of implementations.

\subsection{LLM Adapters}
\label{subsec:llm-adapters}

All LLM providers implement a unified interface:

\begin{itemize}
    \item Ollama Adapter (local inference)
    \item OpenAI Adapter (cloud API)
    \item Azure Adapter (enterprise cloud)
    \item Stub/Demo Adapter (testing)
\end{itemize}

\subsection{Database Adapters}
\label{subsec:db-adapters}

Each database has a dedicated adapter:

\begin{itemize}
    \item \textbf{Redis Adapter}: cache.get/set, queue.push/pop, ratelimit.check, lock.acquire
    \item \textbf{PostgreSQL Adapter}: repository.CRUD, transaction.begin/commit, query.execute
    \item \textbf{Memgraph Adapter}: graph.query, graph.secure\_query, graph.nl\_to\_cypher
\end{itemize}

%------------------------------------------------------------------------------
\section{Layer 11: Resilience Framework}
\label{sec:layer11-resilience}

The resilience framework ensures system stability under adverse conditions.

\subsection{Circuit Breaker Pattern}
\label{subsec:circuit-breaker}

Per-provider circuit breakers (stored in Redis) follow three states:

\begin{enumerate}
    \item \textbf{CLOSED}: Normal operation, all requests pass through
    \item \textbf{OPEN}: After N consecutive failures, reject immediately with fallback
    \item \textbf{HALF-OPEN}: After timeout (60s), allow one probe request
\end{enumerate}

Transitions: Success in HALF-OPEN $\rightarrow$ CLOSED; Failure $\rightarrow$ OPEN.

\subsection{Retry Strategy}
\label{subsec:retry-strategy}
...
\subsection{Provider Fallback Chain}
\label{subsec:fallback-chain}

When the primary provider fails, the system attempts fallbacks in order:

\begin{enumerate}
    \item Primary: OpenAI GPT-4
    \item Fallback 1: Azure GPT-4
    \item Fallback 2: Ollama (local)
    \item Return Error (all failed)
\end{enumerate}

\subsection{Cost Tracking}
\label{subsec:cost-tracking}

The system tracks LLM costs:

\begin{itemize}
    \item \texttt{tokens\_input}: Tokens in prompt (billing)
    \item \texttt{tokens\_output}: Tokens in response (billing)
    \item \texttt{cost\_usd}: Estimated cost (budgets)
    \item \texttt{latency\_ms}: Response time (performance)
\end{itemize}

%------------------------------------------------------------------------------
\section{Layer 12: Background Framework}
\label{sec:layer12-background}

APScheduler manages periodic maintenance tasks.

\subsection{Scheduled Tasks}
\label{subsec:scheduled-tasks}

Table~\ref{tab:scheduled-tasks} lists the background tasks.

\begin{table}[htbp]
\centering
\caption{APScheduler background tasks}
\label{tab:scheduled-tasks}
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Task} & \textbf{Frequency} & \textbf{Description} \\
\midrule
Health Check & 30 seconds & Ping databases, check LLM availability \\
Cleanup & Hourly & Remove stale sessions, clear cache, archive jobs \\
Backup & Daily & PostgreSQL dump, Redis snapshot, audit export \\
Provider Monitor & 60 seconds & Check health, update circuit breakers \\
\bottomrule
\end{tabular}
\end{table}

%------------------------------------------------------------------------------
\section{Layer 13: Observability and Monitoring}
\label{sec:layer13-observability}

Comprehensive observability enables debugging, performance analysis, and operational monitoring.

\subsection{Tracing (OpenTelemetry)}
\label{subsec:otel-tracing}

OpenTelemetry captures distributed traces across the system:

\begin{itemize}
    \item \texttt{agent\_run}: Full execution from prompt to response
    \item \texttt{step\_execution}: Each TODO step within a run
    \item \texttt{tool\_invocation}: Every MCP tool call with inputs/outputs
    \item \texttt{db\_query}: Database operations with timing
    \item \texttt{llm\_call}: LLM requests with model, tokens, latency
    \item \texttt{job\_lifecycle}: Enqueue $\rightarrow$ execute $\rightarrow$ complete/fail/cancel
\end{itemize}

Export destinations: Jaeger, Tempo, Datadog.

\subsection{Prometheus Metrics}
\label{subsec:prometheus-metrics}

Key metric categories:

\begin{itemize}
    \item \textbf{HTTP}: request duration, requests total by endpoint/status
    \item \textbf{Agent Runs}: run duration, run total by status
    \item \textbf{Steps}: step duration, step total by type
    \item \textbf{Tools}: invocation duration, invocation total by tool
    \item \textbf{LLM}: tokens total, cost USD, latency by provider/model
    \item \textbf{Circuit Breaker}: state (0=closed, 0.5=half-open, 1=open)
    \item \textbf{Jobs}: queue depth, processing duration
    \item \textbf{System}: component health (1=healthy, 0=unhealthy)
\end{itemize}

\subsection{Grafana Dashboards}
\label{subsec:grafana-dashboards}

Pre-configured dashboards include:

\begin{itemize}
    \item HTTP Overview: Request rate, latency percentiles, error rate
    \item Agent Runs: Runs/min, success rate, duration, step count
    \item Job Processing: Queue depth, processing time, completion rate
    \item Tool Invocations: Most-used tools, latency, error rates
    \item LLM Providers: Health, circuit breakers, token usage, costs
    \item System Health: Component status, DB pools, memory usage
\end{itemize}

\subsection{Health Endpoints}
\label{subsec:health-endpoints}

Standard Kubernetes-compatible health probes:

\begin{itemize}
    \item \texttt{/v1/health/live}: Is the process running?
    \item \texttt{/v1/health/ready}: Can it serve requests?
    \item \texttt{/v1/health/startup}: Has initialization completed?
    \item \texttt{/v1/health/components}: Per-component status
\end{itemize}
...
The CINECA Agentic Platform architecture demonstrates a comprehensive approach to building enterprise-grade AI agent systems. The 13-layer design provides clear separation of concerns, enabling independent scaling, testing, and evolution of each component. Key architectural decisions include:

\begin{itemize}
    \item Defense-in-depth security with 8 middleware layers
    \item Dual workflow support for different execution patterns
    \item Model-agnostic LLM integration with resilient fallbacks
    \item Three-database architecture optimized for different access patterns
    \item Full observability through OpenTelemetry, Prometheus, and Grafana
\end{itemize}

The architecture supports the platform's goals of security, scalability, and operational excellence while enabling natural language interaction with complex bioinformatics data.
...
The execution flow proceeds through four distinct phases aligned with the orchestration architecture:

\paragraph{Phase 1: Intent Classification}
\begin{itemize}
    \item The prompt is analyzed to determine the appropriate processing mode using the six intent categories: CHAT, GRAPH, SECURITY, ADMIN, DANGEROUS, or EXPLAIN.
    \item Classification uses fast regex and keyword matching with optional LLM fallback for ambiguous cases.
    \item Based on the classified intent, the orchestrator routes to specialized handlers:
    \begin{itemize}
        ...
        \item \texttt{\_handle\_explain\_mode()}: Explanations of previous results or queries
    \end{itemize}
\end{itemize}

\paragraph{Phase 2: TODO Planning}
\begin{itemize}
    \item For complex requests, the orchestrator generates a structured plan as a list of task items.
    \item Planning modes: \textbf{full} (always plan), \textbf{optional} (plan if beneficial), \textbf{none} (skip planning).
    \item Each TODO item represents a discrete step with description, expected outcome, and dependencies.
\end{itemize}

\paragraph{Phase 3: Step Execution}
\begin{itemize}
    \item The orchestrator iterates through planned steps, invoking tools, making LLM calls, and accumulating results.
    \item Each step is recorded with timing metrics, tool inputs/outputs, and success status.
    \item Maximum step limit (default: 25) prevents unbounded execution.
\end{itemize}

\paragraph{Phase 4: Finalization}
\begin{itemize}
    \item Final outputs are aggregated, formatted, and returned to the caller.
    \item Comprehensive metrics captured: token usage, latency, cost estimates, step count.
    \item State persistence: final state written to PostgreSQL and cached in Redis.
\end{itemize}
...
The classifier recognizes six distinct intent categories, each triggering specialized handling in the orchestrator:
...
\begin{description}
    ...
    \item[EXPLAIN] Queries requesting explanations of previous results, Cypher queries, or system behavior. These requests provide educational context without executing additional queries.
\end{description}
...
\subsubsection{EXPLAIN Patterns}

Explain patterns detect requests for explanations of results or queries:

\begin{itemize}
    \item \textbf{Result Explanations}: Questions about previous query results (``explain this result'', ``what does this mean'')
    \item \textbf{Query Explanations}: Requests to explain Cypher queries (``explain this query'', ``how does this work'')
    \item \textbf{System Behavior}: Questions about platform behavior (``why did you'', ``how did you get this'')
\end{itemize}
...
The background framework includes four categories of scheduled tasks, aligned with operational requirements:

\begin{description}
    \item[Health Monitoring (every 30 seconds)] Periodic checks of PostgreSQL ping, Redis ping, Memgraph ping, and LLM provider warmup/availability.
    
    \item[Cleanup (hourly)] Removal of stale/old sessions, expired cache entries, expired jobs, old job records, and orphan agent runs.
    
    \item[Backup (daily)] PostgreSQL dumps, Redis RDB snapshots, Memgraph archives, and audit log exports.
    
    \item[Provider Monitoring (continuous)] Provider health checks and metrics emission for circuit breaker state management.
\end{description}
...
The platform employs comprehensive multi-layer testing with 272 test files, $\sim$64,500 lines of test code, and 3,000+ test functions across 27 categories.
...
The platform ships with 236+ test files, 3,000+ test functions across 27 categories, and $\sim$64,500 lines of test code. Test markers include: \texttt{unit}, \texttt{integration}, \texttt{e2e}, \texttt{performance}, and \texttt{security}.
