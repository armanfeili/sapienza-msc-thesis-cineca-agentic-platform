...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%  SAPTHESIS TEMPLATE FOR:
%%  CINECA Agentic Platform: An Enterprise-Grade Multi-Tenant AI Orchestrator
%%  with MCP Tools and Secure Graph Querying
%%  
...
\hypersetup{
    pdftitle={CINECA Agentic Platform: An Enterprise-Grade Multi-Tenant AI Orchestrator with MCP Tools and Secure Graph Querying},
...
\title{CINECA Agentic Platform:\\
       An Enterprise-Grade Multi-Tenant AI Orchestrator\\
       with MCP Tools and Secure Graph Querying}

\author{Arman Feili}
\IDnumber{2101835}
\course{Master of Science in Data Science}
\courseorganizer{Department of Information Engineering,\\Electronics and Telecommunications}
...
\newcommand{\CategoryCount}{12}
\newcommand{\EndpointCount}{76}
\newcommand{\TestCount}{1,000+}
...
\begin{abstract}
    ...
    (MCP) runtime governs \ToolCount{} tools across \CategoryCount{} official
    high-level tool families with schema validation, role-based access
    control (RBAC), and audit logging.
    ...
    tools, validated by \TestCount{} automated tests spanning unit,
...
This thesis was supervised jointly by academic and industry advisors.
Prof.~Maria De Marsico (Sapienza Universit\`a di Roma) served as
University Advisor, with Prof.~Marco Raoul Marini and Dr.~Valerio
Venanzi (Sapienza Universit\`a di Roma) as University Co-Advisors,
providing academic supervision and methodological guidance throughout
the thesis. The industrial context, requirements, and access to CINECA's
infrastructure were provided under the company supervision of
Dr.~Giuseppe Melfi and Dr.~Marco Puccini (CINECA), who are listed above
as CINECA Advisors. The distinction between these two supervision roles
reflects the dual academic/industrial nature of this thesis project and
is made explicit here for clarity, since the thesis template lists all
five names uniformly as advisor and co-advisors.
\end{acknowledgments}
...
tenant-aware graph querying, and built-in observability.  This work was
developed during my thesis activity with CINECA, where the practical need
for controlled natural-language access to graph data shaped the platform
requirements.
...
\begin{itemize}
    ...
    \item A \textbf{governed MCP tool runtime} hosting 34~tools across
          12~official high-level tool families, each registered with a JSON
          input schema, protected by RBAC and per-tool rate limits, and
...
The platform was validated through \TestCount{} automated tests
...
families---cloud-hosted (OpenAI), enterprise (Azure OpenAI), and locally
self-hosted (Ollama)---demonstrating that the platform can switch
providers at the request, session, or tenant level without code changes.
Through the Ollama adapter, the platform supports local model families
such as Phi-3, Mistral, LLaMA, Qwen, and Gemma; the experimental
evaluation itself, however, used a smaller representative subset of
these supported options---OpenAI \texttt{gpt-4o-mini} and Ollama
\texttt{llama3.2}
(Table~\ref{tab:app-env}, Section~\ref{app:reproducibility})---due to
local hardware and time constraints (Section~\ref{sec:eval-constraints}).
The supported provider and model set is therefore broader than the set
actually exercised in the evaluation reported in this thesis.
...
the presence of a monitoring dashboard or CLI.  For the oral defense, the
presentation deck reports a compressed subset of this comparison---CAP
against Temporal, LangGraph, the OpenAI Agents SDK, Semantic Kernel, and
n8n---to fit the time constraints of the talk; the full ten-platform
comparison in Table~\ref{tab:extended-comparison} below is the complete
version on which that summary is based.
...
A researcher submits a prompt through the chat interface.  The orchestrator
...
knowledge graph.  The pipeline translates the question into a Cypher query,
...
The platform hosts 34~tools across 12~official high-level tool families
...
The platform exposes 34~MCP tools organised into \CategoryCount{} official
high-level tool families (FR-TL-01): \emph{Graph}, \emph{Security},
\emph{System}, \emph{Cache}, \emph{Catalog}, \emph{Model}, \emph{Agent},
\emph{Analytics}, \emph{Admin}, \emph{ETL}, \emph{CRUD}, and \emph{Export}.
The largest family is \emph{graph}, with eight
tools covering secure queries, schema inspection, analytics, and Cypher
generation.  The \emph{security} family provides five tools for
permission checks, audit queries, and principal inspection.  The
\emph{system} family offers four tools for health, status, metrics, and
backup.  The remaining families are smaller (one or two tools each) and
cover model management, output formatting, cache management, catalog
discovery, agent context, and administrative, ETL, CRUD, and export
operations.  Some implementation modules additionally use finer-grained
internal labels (for example, data quality, database switching, error
reporting, privacy consent, rate-limit management, session management,
tenancy management, user profiles, and visualisation) that are grouped
under the closest official family for reporting purposes; the thesis and
the defense presentation both report the platform's tool ecosystem as
34~tools across 12~official high-level tool families.  The complete tool
inventory, including these internal implementation labels, is provided in
...
response, and the denial is audit-logged (NFR-AU-02).  For presentation
purposes, the defense deck simplifies this four-role hierarchy down to a
two-tier admin/user view; the platform itself implements, and this
thesis evaluates, the full four-role model described above.
...
from its external outputs---is treated in CAP as a core design priority
rather than an optional add-on.  CAP
instruments every layer with three complementary signal types: metrics,
logs, and traces (NFR-OB-01 through NFR-OB-04).

\subsection{Metrics}

The runtime exposes Prometheus-compatible~\cite{prometheus2023} metrics
...
\subsection{Scheduled background tasks and observability}
\label{sec:background-tasks}

Beyond the request-time metrics, logs, and traces described above, the
platform runs a set of scheduled background tasks through
APScheduler~\cite{apscheduler2023}, a Python job scheduler that executes
recurring maintenance work without manual intervention.  Four jobs are
scheduled at different frequencies, each addressing a different
operational concern.  A \emph{health-check} job runs every 30~seconds and
pings the relational database, the in-memory store, the graph database,
and the configured LLM providers, so that failures are detected within
seconds rather than at the next user request.  A \emph{provider-monitoring}
job runs every 60~seconds and tracks LLM-provider health, updates circuit-breaker
states, and emits the corresponding Prometheus metrics
(Section~\ref{sec:observability}).  An hourly \emph{cleanup} job removes
expired sessions, stale cache entries, completed or failed jobs older than
the retention period, and orphaned agent runs, preventing unbounded growth
of the control-plane database.  A daily \emph{backup} job creates recovery
snapshots of the relational database, the in-memory store, and the graph
database, and exports audit-log snapshots for compliance.

These scheduled tasks are a first-class part of the platform's
observability posture, not a separate concern: the health-check and
provider-monitoring jobs feed directly into the same Prometheus metrics
and Grafana dashboards described in Section~\ref{sec:observability},
the OpenTelemetry traces (this section) capture the execution of each
scheduled job as its own span, and the cleanup and backup jobs are
themselves audit-logged.  Together, the OpenTelemetry tracing,
Prometheus metrics, Grafana dashboards, and APScheduler-driven health
checks, cleanup, and backups give operators continuous visibility into
platform health and a documented recovery path if a component degrades.
...
The adapter layer abstracts provider-specific APIs---cloud-hosted
(OpenAI), enterprise (Azure OpenAI), and locally self-hosted (Ollama,
supporting local model families such as Phi-3, Mistral, LLaMA, Qwen, and
Gemma)---behind a common interface, allowing the platform to switch
providers at the request, session, or tenant level without code changes
...
The platform ships with a test suite comprising \TestCount{}
automated test cases, distributed across test functions and test files
spanning tens of thousands of lines of test code.  Tests are organised into 27 categories that mirror
...
\begin{tabularx}{\textwidth}{@{}l l X@{}}
    ...
    EC-FC-06 & FR-TL-01, 02, 03 & $\geq$\,30 tools with valid JSON
        schemas across the 12 official tool families \\
...
The test suite comprises \TestCount{} automated test cases
distributed across the unit, integration, security, and end-to-end
categories described in Section~\ref{sec:test-suite-overview}.
...
\subsection{End-to-End NL-to-Graph Validation Example}
\label{sec:blast-version-example}

Beyond the aggregate accuracy figures reported above, a single concrete
trace illustrates how a user request travels through the full
NL-to-Cypher pipeline described in Section~\ref{sec:nl-cypher}.  The
example below is a real end-to-end run captured from the deployed
platform and reproduced in the defense presentation; it is included here
because it demonstrates, step by step, the same five-stage flow---intent
classification, Cypher generation, safety validation, graph execution,
and final summarisation---that the aggregate benchmark exercises at
scale.

\paragraph{User request.}
The user asks, in plain natural language: \emph{``Count Blast nodes
grouped by presence of \texttt{blast\_version}.''}

\paragraph{1. Intent classification.}
The orchestrator's intent classifier (Section~\ref{sec:orchestration-engine})
routes the prompt to the \textsc{graph} intent category, since the
request asks a quantitative question over the graph database rather than
requesting a conversational answer, an administrative action, or a
tool-invocation side effect.

\paragraph{2. Cypher generation.}
The pipeline generates a read-only, parameterised Cypher query that
groups \texttt{Blast} nodes by whether the \texttt{blast\_version}
property is present:
\begin{quote}
\ttfamily\small
MATCH (b:Blast)\\
RETURN b.blast\_version IS NOT NULL AS has\_blast\_version,\\
\phantom{RETURN }count(b) AS count\\
\phantom{RETURN }ORDER BY has\_blast\_version DESC
\end{quote}

\paragraph{3. Safety validation.}
The generated query is passed through the six-layer safety validator
(Section~\ref{sec:nl-cypher}) before execution.  It is confirmed to be
read-only (no \texttt{CREATE}, \texttt{SET}, or \texttt{DELETE} clauses),
tenant-safe (scoped to the caller's tenant), within the configured
traversal-depth limit, and timeout-protected; the query passes safety
validation on all four checks and is admitted for execution.

\paragraph{4. Memgraph execution.}
The validated query is executed against the graph database through the
tenant-isolated \texttt{graph.secure\_query} tool
(Section~\ref{sec:mcp-tools}).  Because the query aggregates by a single
boolean grouping key, execution returns exactly one grouped row.

\paragraph{5. Final summary.}
The orchestrator's summarisation stage converts the single grouped result
into a natural-language answer returned to the user:
\emph{``In total, there are 39 Blast nodes in the database.  All
containing the \texttt{blast\_version} attribute.''}

This trace is representative of the catalog-matched and LLM-generated
queries that make up the 83\% full-loop accuracy figure reported above:
the query is short, maps directly onto a single node label and property,
and does not require the multi-hop traversal or informal-synonym
resolution that account for most of the pipeline's residual failures.

\subsection{Tool registry (EC-FC-06)}

The platform registers 34~tools across \CategoryCount{} official
high-level tool families (FR-TL-01, FR-TL-02).  Integration tests verify that each registered
...
schema validation, exceeding the $\geq$\,30 tools threshold set by
EC-FC-06 while covering all 12 official tool families.
...
\subsection{Evaluation constraints and future validation needs}
\label{sec:eval-constraints}

Before the detailed internal-, external-, and construct-validity
discussion below, this section states, at the level presented at the
oral defense, the constraints that most directly bound the evaluation's
scope and the platform's day-to-day usability.

\paragraph{HPC availability.}
An evaluation against CINECA's high-performance computing (HPC)
infrastructure was planned but was not available within the timeframe of
this thesis.  All results reported in this chapter were obtained on the
single-node development environment described in
Section~\ref{sec:experimental-setup}, not on production HPC hardware.

\paragraph{Limited scenarios.}
The functional, performance, security, and NL-to-Cypher evaluations
exercise a limited set of representative scenarios and benchmark
queries rather than the full space of prompts and workloads that a
production deployment would eventually encounter.

\paragraph{Local model latency.}
During development and evaluation, more powerful local (Ollama-hosted)
models proved too slow to use interactively on the laptop-class hardware
described in Section~\ref{sec:experimental-setup}, which constrained
which local models could realistically be exercised end-to-end.

\paragraph{Long run time.}
Multi-step agent runs and jobs that chain several LLM calls, tool
invocations, and graph queries could take more than twenty minutes to
complete end-to-end on the test hardware, which is an important
practical constraint for interactive use and for repeated evaluation
runs.

\paragraph{Model trade-off.}
Given the latency constraint above, the evaluation standardised on a
lightweight model (Section~\ref{sec:experimental-setup}) that offered an
acceptable balance between response speed and answer quality, rather
than the largest available model.

\paragraph{Prompt handling.}
Unclear, ambiguous, or potentially risky prompts are not yet handled with
a dedicated strategy beyond the existing \textsc{dangerous} intent
category (Section~\ref{sec:orchestration-engine}); more nuanced handling
of borderline requests remains an open problem, addressed further in
Chapter~\ref{ch:future-work}.

\paragraph{Clarification logic.}
The orchestrator does not yet ask clarifying questions when a prompt is
under-specified; it currently falls back to a safe default behaviour
(for example, routing ambiguous prompts to the chat handler,
Section~\ref{sec:functional-results}) rather than requesting
disambiguation from the user, and this clarification logic and its safe
defaults need further refinement.

\paragraph{LLM variability.}
Because the orchestration engine and the NL-to-Cypher pipeline both rely
on LLM generation, different model choices---or different versions of
the same model---may produce different plans, tool-call sequences, or
Cypher queries for the same prompt, which is a source of variability that
the deterministic test mode (Section~\ref{sec:functional-protocol})
only partially controls for.

\paragraph{Residual risk.}
The six-layer NL-to-Cypher safety validation and the eight-stage security
middleware stack substantially reduce the risk of unsafe or unauthorised
behaviour, but---consistent with the security results in
Section~\ref{sec:security-results}---they do not, and are not claimed to,
remove that risk entirely; residual risk from LLM variability, novel
prompt phrasings, and threats outside the defined threat model remains.
...
The comprehensive test coverage (\TestCount{} automated test cases
...
The MCP tool runtime hosts 34~tools across \CategoryCount{} official
high-level tool families.
...
Observability is treated as a core requirement rather than an
afterthought: over 50~Prometheus metrics, six
...
The architecture is LLM-agnostic.  Provider adapters abstract the differences
...
The test suite comprises \TestCount{} automated test cases
spanning unit, integration, security, and end-to-end scenarios.  The overall
...
integrated observability infrastructure were validated through
\TestCount{} automated tests, a structured security audit, and a comparative
...
\paragraph{Ask Mode.}
A complementary extension, and one of the most immediately impactful
items on this platform's future-work agenda, is the introduction of a
lightweight ``Ask Mode''---a user-facing default configuration that sets
the existing planning mode to ``none'' and disables tool calls unless
explicitly requested, thereby providing a non-agentic, direct
question-answering path for simple factual queries (e.g., ``How many
workflows are there?'').  This path would still enforce the eight-stage
security middleware stack but would skip the planning phase entirely,
reducing latency for single-step questions and lowering LLM costs for
queries that do not require multi-step reasoning.

\paragraph{Handling unclear, mixed, and risky prompts.}
The evaluation constraints noted in Section~\ref{sec:eval-constraints}
(prompt handling, clarification logic) point to a second priority: making
the orchestrator's handling of imperfect prompts more autonomous and
reliable, from easy and unambiguous requests through to ambiguous,
multi-intent, and risky ones.  Concretely, an \emph{ambiguous} prompt
such as ``Show top collaborations'' should either trigger a targeted
clarifying question (e.g., ``top by what metric?'') or fall back to a
documented safe default rather than guessing silently; a
\emph{multi-intent} prompt such as ``Find top institutions and export to
CSV'' should be decomposed into an explicit plan (query, then export)
rather than only the first intent being honoured; and a \emph{risky}
prompt such as ``Delete failed jobs'' should be refused or routed through
an explicit admin-confirmation step, extending the existing
\textsc{dangerous}-intent handling
(Section~\ref{sec:orchestration-engine}) with a general-purpose
clarification-and-safe-default layer rather than a fixed set of
special-cased rules.  Because different LLM providers and model sizes
may classify or answer the same borderline prompt differently
(Section~\ref{sec:eval-constraints}), this clarification logic should be
evaluated across multiple providers before being relied upon in
production.
...
step, because step-level checkpointing is not implemented.  Five
...
\emph{Recover interrupted workflows from the last completed step.}
Currently, when a worker crashes mid-job, the supervisor detects the
orphaned job via the heartbeat mechanism and re-enqueues it, but the
recovered job restarts from Phase~1 rather than from the interrupted step
(EC-RE-07).  Persisting a checkpoint after each step (recording which
to-do items have been completed and their outputs) would let the platform
recover interrupted workflows from the last completed step instead of
restarting the entire multi-step run, significantly reducing recovery
time for long-running jobs.

\emph{Priority-aware job scheduling.}  The job queue currently treats all
jobs of a given type equally, in strict first-in-first-out order.
Managing job queues with priorities, per-tenant concurrency limits, and
overload protection would let the platform make better use of shared
capacity under load: short, interactive jobs (e.g., chat-style agent
runs) could be prioritised ahead of long-running batch or export jobs, and
per-tenant caps (for example, Tenant~A limited to twenty concurrent jobs
while Tenant~B is limited to five) would prevent a single noisy tenant
from starving the others.  Combined with backpressure signalling when the
queue depth approaches capacity, this would give operators the overload
protection that the current, unbounded FIFO queue does not provide.

\emph{Additional LLM providers.}  The adapter layer currently supports
three provider families---cloud-hosted (OpenAI), enterprise (Azure
OpenAI), and locally self-hosted (Ollama, with local model families such
as Phi-3, Mistral, LLaMA, Qwen, and Gemma)---of which the evaluation in
Chapter~\ref{ch:results} exercised only a smaller representative subset
(\texttt{gpt-4o-mini} and \texttt{llama3.2}).  Extending the adapter
layer with additional commercial and on-premises providers, and
evaluating a broader set of the already-supported local model families,
would broaden deployment flexibility and strengthen the fallback chain by
offering more alternatives when one provider is degraded.
...
path and adding a lightweight Ask Mode, strengthening prompt handling
(unclear, mixed, and risky prompts) with clarification logic and safe
defaults alongside the NL-to-Cypher pipeline's embedding-based matching
and schema-evolution awareness, extending agent capabilities
with multi-agent collaboration, advanced memory, and document RAG,
hardening the infrastructure by recovering interrupted workflows from
the last completed step, adding priority-aware job scheduling with
tenant limits and overload protection, and distributing the worker
architecture, and deepening the evaluation with production-environment
...
% \chapter* does not step the chapter counter, so \thechapter is left
% empty by \appendix (which sets \c@chapter to 0); with the default
% \thetable = \thechapter.\arabic{table}, this rendered as "Table .1"
% instead of "Table A.1".  Number tables and figures by appendix section
% (A, B, C, ...) instead, resetting each counter whenever a new section
% starts.
...
% \chapter* also does not call \chaptermark, so the even-page running
% header would otherwise keep showing the previous numbered chapter
% (e.g. "7. Future Work") for the whole appendix, even though the
% odd-page header is correctly updated by \sectionmark at each
% \section.  Set both marks explicitly so the appendix headers are
...
The platform exposes 34 Model Context Protocol (MCP) tools (EC-FC-06
requires at least 30 tools with valid JSON schemas across the 12
official tool families).  Each tool is registered under a
...
Table~\ref{tab:app-tool-inventory} lists every tool together with its
implementation-level category label, required scope, and a brief
functional description.  These implementation-level labels are a
finer-grained internal breakdown; for headline reporting purposes
(Section~\ref{sec:mcp-tools}), they are grouped into the \CategoryCount{}
official high-level tool families reported throughout this thesis and in
the defense presentation: Graph, Security, System, Cache, Catalog, Model,
Agent, Analytics, Admin, ETL, CRUD, and Export.

{\footnotesize
\begin{longtable}{@{}l l l p{4.0cm}@{}}
\caption{Complete MCP tool inventory (34~tools; implementation-level
category labels shown, grouped into 12~official high-level tool
families).}
...
agent-run and job lifecycles in prose.  Because the two workflows share
the same orchestration engine (Section~\ref{sec:orchestration-engine}),
their state machines are structurally identical: both follow the same
five-state topology with the same four transitions.  Only two labels
differ---the success state is called \emph{succeeded} for agent runs
and \emph{finished} for jobs, and the first transition is \emph{start}
(synchronous dispatch) versus \emph{dequeue} (worker picks up from the
queue).  Figure~\ref{fig:lifecycle-sm} captures the common structure,
with the workflow-specific labels shown in parentheses.

\begin{figure}[ht]
\centering
\begin{tikzpicture}[
    st/.style={rectangle, rounded corners=3pt, draw, fill=gray!8,
        minimum width=2.6cm, minimum height=0.6cm, font=\small},
    tm/.style={st, double, double distance=1pt},
    ar/.style={->, >=latex, thick},
    node distance=1.4cm and 2.4cm]
\node[st] (Q) {queued};
\node[st, right=of Q] (R) {running};
\node[tm, above right=0.9cm and 2.4cm of R] (S) {succeeded\,/\,finished};
\node[tm, right=of R] (F) {failed};
\node[tm, below right=0.9cm and 2.4cm of R] (C) {cancelled};
\draw[ar] (Q) -- node[above, font=\scriptsize] {start\,/\,dequeue} (R);
...
\end{tikzpicture}
\caption{Unified lifecycle state machine for agent runs (FR-AE-01,
    EC-FC-01) and jobs (FR-JB-02, EC-FC-07).  Labels before the slash
    apply to agent runs; labels after the slash apply to jobs.
    Double-bordered states are terminal.  For jobs, cancellation is
    cooperative: the worker checks a cancellation flag between steps.}
\label{fig:lifecycle-sm}
\label{fig:run-sm}
\label{fig:job-sm}
\end{figure}

Both workflows begin in the \emph{queued} state.  An agent run is
queued when the API accepts a prompt and dispatched immediately
(\emph{start}); a job is queued when its identifier is persisted in the
control-plane database and pushed onto the in-memory store, then picked
up by a dedicated worker (\emph{dequeue}).  From \emph{running}, three
terminal transitions are possible: the success state (\emph{succeeded}
or \emph{finished}) on normal completion, \emph{failed} on an
unrecoverable error, or \emph{cancelled} when a client-initiated
cancellation request is honoured.  Either workflow may also move
directly from \emph{queued} to \emph{cancelled} if the cancellation
arrives before execution begins.  During the \emph{running} state a job
worker additionally emits heartbeat updates; if the heartbeat ceases, a
supervisor detects the orphaned job and re-enqueues it (NFR-RE-06).
...
The full test suite (\TestCount{} tests at the snapshot commit) is
executed through a single invocation of the test runner.  Tests are organised
